...
- После изменения кода запускай `cargo check` / `svelte-check` / `pyright` / `tsc` / `mix compile` (согласно стеку).
...
1. **LSP Validation Loop** — после каждой правки файла дожидайся диагностики.
...
2. **Правило «Первых трёх»** — при большом количестве ошибок исправляй 3-5 корневых,
   затем запрашивай новую диагностику. Не пытайся закрыть все 50 сразу.
3. **Skills** — используй подключённые скиллы (BPMN, LSP-coding и др.) согласно их описанию.
...
- [ ] Соответствует ли стиль кода конвенциям проекта (раздел 9)?
...
- Задача затрагивает архитектуру или публичный API.
...
- **Ссылки:** Техническое задание v3.0
...
|---|---|---|---|---|
| **ADR-001** | Обработка обрывов TCP | Reconnect + очистка памяти в GenServer | Протокол не гарантирует доставку, сессии должны умирать чисто | ✅ Принято |
| **ADR-002** | Хранение паролей | Argon2id, без самописных хешей | Требования безопасности, устойчивость к брутфорсу | ✅ Принято |
| **ADR-003** | Формат обмена с эмулятором | JSON по HTTP/WS, бинарный протокол только для TCP | Удобство отладки на этапе разработки UI | ✅ Принято |
| **ADR-004** | Шифрование ГОСТ | Заглушка, интеграция с КриптоПро позже | КриптоПро требует лицензии и специфичного окружения | ⏳ Отложено |
